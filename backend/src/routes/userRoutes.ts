import { Router, Request, Response, NextFunction } from 'express';
import { UserController } from '../controllers/UserController';
import { authMiddleware } from '../middlewares/authMiddleware';
import { AuthRequest } from '../interfaces/AuthRequest';

const router = Router();
const userController = new UserController();

router.get('/', (req: Request, res: Response) => {
    res.json({ message: 'Rutas de usuarios configuradas' });
});

// rutas publicas
router.post('/register', (req: Request, res: Response, next: NextFunction) => {
    userController.register(req as AuthRequest, res).catch(next);
});

router.post('/login', (req: Request, res: Response, next: NextFunction) => {
    userController.login(req as AuthRequest, res).catch(next);
});

router.post('/forgot-password', (req: Request, res: Response, next: NextFunction) => {
    userController.forgotPassword(req as AuthRequest, res).catch(next);
});

router.post('/reset-password', (req: Request, res: Response, next: NextFunction) => {
    userController.resetPassword(req as AuthRequest, res).catch(next);
});

// rutas publicas con autenticacion
router.get('/profile', authMiddleware, (req: AuthRequest, res: Response, next: NextFunction) => {
    console.log('Recibida petición de perfil para usuario:', req.user?.id);
    userController.getProfile(req, res).catch(next);
});

router.put('/profile', authMiddleware, (req: AuthRequest, res: Response, next: NextFunction) => {
    console.log('Recibida petición de actualización de perfil para usuario:', req.user?.id);
    userController.updateProfile(req, res).catch(next);
});

router.put('/change-password', authMiddleware, (req: AuthRequest, res: Response, next: NextFunction) => {
    console.log('Recibida petición de cambio de contraseña para usuario:', req.user?.id);
    userController.changePassword(req, res).catch(next);
});

console.log('Rutas de usuarios configuradas');

export default router; 