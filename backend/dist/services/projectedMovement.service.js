...
exports.ProjectedMovementService = void 0;
const connection_1 = require("../config/database/connection");
const plan_service_1 = require("./plan.service");
const errors_1 = require("../utils/errors");
class ProjectedMovementService {
    constructor() {
        this.pool = connection_1.pool;
        this.planService = new plan_service_1.PlanService();
    }
    async countProjectedByUser(userId) {
        const res = await this.pool.query(`SELECT COUNT(*) AS cnt FROM projected_movements WHERE user_id = $1`, [userId]);
        return Number(res.rows[0].cnt);
    }
    async getAllProjectedMovements(userId) {
        const query = `
            SELECT 
    ...
    async getProjectedMovementById(id, userId) {
        const query = `
            SELECT 
                pm.id, pm.user_id as "userId", pm.category_id as "categoryId",
                pm.card_id as "cardId", pm.amount, pm.description,
    ...
    async getProjectedMovementsByFilters(filters) {
    ...
    async createProjectedMovement(params) {
        const { userId, planId, categoryId = null, cardId = null, amount, description = null, movementType, expectedDate, probability = 100, recurrenceType = null } = params;
        try {
            ...
            if (limits.projected_movements !== undefined && limits.projected_movements !== -1) {
                ...
                if (used >= limits.projected_movements) {
                    throw new errors_1.DatabaseError(`Has alcanzado el límite de ${limits.projected_movements} movimientos proyectados para tu plan`);
                }
            }
        }
        catch (error) {
            ...
            throw new errors_1.DatabaseError('Error al obtener los límites del plan');
        }
        const query = `
    ...
    async updateProjectedMovement(id, userId, movementData) {
    ...
    async deleteProjectedMovement(id, userId) {
     ...
     */
    async getIntelligentProjectedMovements(userId) {
        try {
     ...
     */
    async checkRealMovements(userId) {
        const query = `
     ...
     */
    async generateAutoProjectedMovements(userId) {
        try {
    ...
    getProjectedMovementSelectFields() {
        return `
          pm.id,
...
exports.ProjectedMovementService = ProjectedMovementService;
//# sourceMappingURL=projectedMovement.service.js.map