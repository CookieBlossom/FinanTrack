...
const projectedMovement_service_1 = require("./projectedMovement.service");
...
class AutomationService {
     ...
     */
    async processOverdueProjectedMovements() {
        const result = {
            processed: 0,
            skipped: 0,
            errors: 0,
        ...
        try {
            ...
            throw new errors_1.DatabaseError('Error al procesar movimientos proyectados automáticamente');
        }
        return result;
    }
    /**
     ...
     */
    async getOverdueHighProbabilityMovements() {
        const query = `
            SELECT 
     ...
     */
    async shouldProcessMovement(movement) {
     ...
     */
    async checkExistingMovement(movement) {
        const query = `
     ...
     */
    async getLastProcessedRecurringMovement(movement) {
        const query = `
     ...
     */
    async convertToRealMovement(movement) {
        ...
        try {
            ...
            await this.projectedMovementService.updateProjectedMovement(movement.id, movement.userId, {
                status: 'completed',
     ...
     */
    async createNextRecurringMovement(movement) {
     ...
     */
    calculateNextRecurrenceDate(currentDate, recurrenceType) {
