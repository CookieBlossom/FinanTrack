...
class AnalyticsService {
    ...
    async checkAnalyticsPermission(planId) {
        const hasPermission = await this.planService.hasPermission(planId, 'advanced_analytics');
        if (!hasPermission) {
            throw new errors_1.DatabaseError('Tu plan no incluye acceso a analytics avanzados.');
        }
    }
    // Verificar permisos de exportación de datos
    async checkExportPermission(planId) {
        const hasPermission = await this.planService.hasPermission(planId, 'export_data');
        if (!hasPermission) {
            throw new errors_1.DatabaseError('Tu plan no incluye exportación de datos.');
        }
    }
    ...
    async getAnalyticsDataByMonth(userId, year, month) {
        try {
    ...
    hasAnyData(chartData) {
        return (chartData.realTransactions.length > 0 ||
    ...
    async getMonthlySummaryByMonth(userId, year, month) {
        try {
     ...
     */
    async generateAutoProjections(userId) {
        try {
     ...
     */
    consolidateProjections(projections) {
